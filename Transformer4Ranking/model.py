...
from baseline_model.ranking_model.DNN import DNN
...
class PositionalEncoding(nn.Layer):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 513):
        ...
        # import ipdb
        # ipdb.set_trace()
        pe[:, 0, 0::2] = paddle.sin(paddle.matmul(paddle.ones(
            [max_len, 1]), paddle.reshape(div_term, (1, -1))))
        pe[:, 0, 1::2] = paddle.cos(paddle.matmul(paddle.ones(
            [max_len, 1]), paddle.reshape(div_term, (1, -1))))
...
def torch_padding_mask_to_paddle(padding_mask, n_head):
    ...
    mask = paddle.to_tensor(paddle.unsqueeze(
        padding_mask, axis=1), dtype=paddle.float32)
    mask = paddle.to_tensor(paddle.einsum(
        "ijk, ijl -> ikl", mask, mask), dtype=paddle.int64)  # [bs, seq_len, seq_len]

    paddle_mask = paddle.unsqueeze(mask, 1)
    new_shape = paddle_mask.shape[:1] + [n_head] + paddle_mask.shape[2:]
    # [bs, n_head, seq_len, seq_len]
    paddle_mask = paddle.expand(paddle_mask, new_shape)
    return paddle_mask


class TransformerModel(nn.Layer):
    def __init__(self, ntoken, hidden, nhead, nlayers, dropout, mode='finetune', combine=False, projection=False, rank_feature_size=None, add_freq=False):
        super().__init__()
        ...
        if mode == 'pretrain':
        ...
        elif mode == 'finetune_point':
            self.act = nn.ELU(alpha=1.0)
            self.fc1 = nn.Linear(hidden, 512)
            self.fc2 = nn.Linear(512, 256)
            self.fc3 = nn.Linear(256, 128)
            self.fc4 = nn.Linear(128, 1)
        elif mode == 'finetune_pair' or mode == 'both':
            self.act = nn.ELU(alpha=1.0)
            self.fc1 = nn.Linear(hidden, 512)
            self.fc2 = nn.Linear(512, 256)
            self.fc3 = nn.Linear(256, 128)
            self.fc4 = nn.Linear(128, 1)
        elif mode == 'task1_finetune':
            self.act = nn.ELU(alpha=1.0)
            self.fc1 = nn.Linear(hidden, 512)
            self.fc2 = nn.Linear(512, 256)
            self.fc3 = nn.Linear(256, 128)
            self.fc4 = nn.Linear(128, 1)
            if combine:
                self.ranking = DNN(projection, rank_feature_size)

    def forward(self, src, src_segment, src_padding_mask=None, features=None, mlm_label=None):
        # import ipdb
        # ipdb.set_trace()
        ...
        padding_mask = torch_padding_mask_to_paddle(
            src_padding_mask, self.n_head)
        # x: [256, 128, 768], padding_mask:[256, 12, 128, 128]
        ...
        if self.mode == 'pretrain':  # for train
            ...
            if self.training:
                # shape = [bs, seq_len, num_tokens]
                logits = self.to_logics(output.transpose([2, 0, 1]))
            ...
            scores = paddle.squeeze(scores, axis=-1)
            return scores
        elif self.mode == 'finetune_pair' or self.mode == 'both':
            h1 = self.act(self.fc1(X))
            h1 = self.dropout(h1)
            h2 = self.act(self.fc2(h1))
            h2 = self.dropout(h2)
            ...
            h3 = self.dropout(h3)
            scores = self.fc4(h3)
            scores = paddle.squeeze(scores, axis=-1)  # [bs]
            return scores
        elif self.mode == 'task1_finetune':  # no mask
            if self.combine:
                input = paddle.concat((features, X), axis=1)
                scores = self.ranking(input)
                return scores
            h1 = self.act(self.fc1(X))
            h1 = self.dropout(h1)
            h2 = self.act(self.fc2(h1))
